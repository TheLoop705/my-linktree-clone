...
const PageComponentCanvas: React.FC<PageComponentCanvasProps> = ({
    ...
    return (
      <div className="flex items-center justify-center h-full border-2 border-dashed border-gray-300 rounded-lg">
        <p className="text-gray-500">
          Your page is empty. Drag components from the library to start building.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4 space-y-4">
      <h2 className="text-lg font-semibold text-gray-700 mb-2">Canvas for Page: {pageId}</h2>
      {components.map((component) => (
        ...
        >
          <p className="font-medium text-sm text-gray-800">Type: {component.type}</p>
