...
import { Trash2, ExternalLink, Plus, Settings, Eye, GripVertical } from "lucide-react";
...
const THEME_OPTIONS = [
  ...
  { value: 'dark', label: 'Dark Mode', bg: 'bg-gray-900', accent: 'bg-purple-600' },
  { value: 'gradient', label: 'Gradient', bg: 'bg-gradient-to-br from-pink-500 to-orange-400', accent: 'bg-white' },
  ...
  const updateTheme = async (newTheme: string) => {
    ...
    try {
      const response = await fetch(`/api/pages/${userPage.id}`, {
        ...
        setUserPage(prev => prev ? { ...prev, theme: { themeName: newTheme } } : null);
        toast({
          title: "Success",
    ...
    <div className="max-w-4xl mx-auto space-y-8">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">{userPage.title || "Untitled Page"}</p>
              </div>
              <div>
                <Label htmlFor="page-description">Description</Label>
                <p className="text-sm text-gray-600 mt-1">{userPage.description || "No description"}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      {userPage && (
        <Card>
          ...
          <CardContent>
            <div className="flex gap-4">              {THEME_OPTIONS.map(option => (
                <Button
                  key={option.value}
                  variant={userPage.theme?.themeName === option.value ? "default" : "outline"}
                  className="flex-1"
          ...
          {isAddingLink && (
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <Label htmlFor="link-description">Description (optional)</Label>                  <Textarea
                    id="link-description"
                    placeholder="Brief description of this link"
                    value={newLink.description}
                    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setNewLink(prev => ({ ...prev, description: e.target.value }))}
                  />
                </div>
                <div className="flex gap-2">
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">                          <Switch
                            ...
                            onCheckedChange={(checked: boolean) => toggleLinkStatus(link.id, checked)}
                          />
                          <Button
                            variant="ghost"
                            size="sm"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold">{userPage?.links?.length || 0}</div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold">{userPage?.isPublic ? 'Public' : 'Private'}</div>
